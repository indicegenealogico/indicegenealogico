...
def index(request):
    ...
    regions = Location.objects.raw('''SELECT (l.id) AS id, l.county_state_province AS name, c.name AS country_name,
...
def descendants(request, person_id):
    person = get_object_or_404(Person, id=person_id)
    title = 'Descendientes de ' + person.name()
...
def ancestors(request, person_id):
    person = get_object_or_404(Person, id=person_id)
    title = 'Antepasados de ' + person.name()
...
def ancestors_report(request, person_id):
    ...
    title = 'Antepasados conocidos de ' + person.name()
    return render(request,
                  'people/report.html',
                  {'title': title,
                   'person': person,
...
def ancestors_report_undead(request, person_id):
    ...
    return render(request, 'people/people.html', {'title': title, 'people': people, 'list': Person.objects.select_related('birth')})
...
def ring_chart(request, person_id):
    person = get_object_or_404(Person, id=person_id)
    return render(request, 'people/ringchart.html',  {'person': person, 'list': Person.objects.select_related('birth')}) 
...
def ring_chart_svg(request, person_id):
    ...
    while len(rings) < 10:
        ...
        rings.append(ring)
    return render(request, 'people/ringchart.svg', {'rings': list(reversed(rings))}, content_type='image/svg+xml')
...
def _people_map(request, people, title):
    ...
    return render(request, 'people/map.html', {'title': title, 'locations': counts.keys(), 'map_area' : ((min_lat, min_lng), (max_lat, max_lng)),  'list': Person.objects.select_related('birth')})


def descendants_tree(request, person_id):
    person = get_object_or_404(Person, id=person_id)
    return render(request, 'people/tree.html', {'person': person, 'list': Person.objects.select_related('birth')})
...
def location(request, location_id):
    ...
    buried_ids = Event.objects.filter(event_type=Event.BURIAL, location=location).values_list('person', flat=True)
    burials = Person.objects.filter(id__in=buried_ids)
    return render(request, 'people/location.html', {
                   'location': location,
                   'births': births,
                   'marriages': marriages,
                   'deaths': deaths,
...
def region(request, region_name):
    ...
    title = 'Personas nacidas en ' + region_name
    return render(request, 'people/people.html', {'title': title, 'people': people,  'list': Person.objects.select_related('birth')})
...
def unknown_maiden_names(request):
    ...
    return render(request, 'people/people.html', {'title': 'Married women with unknown maiden names', 'people': people, 'list': Person.objects.select_related('birth')})


def unexpanded_initials(request):
    people = [p for p in Person.objects.all() if len([i for i in p.middle_names.split() if len(i) == 1]) > 0]
    return render(request, 'people/people.html',  {'title': 'People with unexpanded initials', 'people': people, 'list': Person.objects.select_related('birth')})


def undead(request):
    people = Person.objects.filter(deceased=True, death=None)
    return render(request, 'people/people.html',  {'title': 'Deceased people with unknown death details',  'people': people, 'list': Person.objects.select_related('birth')})
...
def start(request):
    with connection.cursor() as cursor:
        ...
        cursor.execute('''SELECT s AS surname FROM
                          (SELECT surname AS s, COUNT(1) AS n
...
def apellidos(request):
    return render(request, 'start/apellidos.html')


def nombres(request):
    return render(request, 'start/nombres.html')